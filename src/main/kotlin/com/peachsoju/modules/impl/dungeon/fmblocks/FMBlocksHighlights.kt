...
object FMBlocksHighlights {
    ...
    data class BlockHighlight(
        ...
        fun copy() = BlockHighlight(itemId, enabled, color.copy(), etherActivate, esp)
    }
    ...
    fun setEsp(itemId: String, enabled: Boolean) {
        highlights[itemId]?.let {
            it.esp = enabled
            save()
        }
    ...
    fun getEspHighlights(): List<BlockHighlight> {
        ensureLoaded()
        return highlights.values.filter { it.enabled && it.esp }
    }
    ...
    fun load() {
        ...
        array.forEach { element ->
            ...
            val esp = obj.get("esp")?.asBoolean ?: false

            highlights[itemId] = BlockHighlight(itemId, enabled, HighlightColor(r, g, b, a), etherActivate, esp)
        }
    }
}