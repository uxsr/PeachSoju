...
object Autoroutes {
    ...
    fun onTick(event: TickEvent.Start) {
        ...
        if (RouteState.awaitingSecrets > 0 || RouteState.awaitingSecretConfirmation || BatListener.isAwaitingBat() || DungeonBreakerListener.isAwaitingDb() || NpcListener.isAwaitingNpc()) {            val leftClickDown = mc.options.keyAttack.isDown
            if (leftClickDown && !leftClickWasDown) {
                when {
                    ...
                    NpcListener.isAwaitingNpc() -> NpcListener.manualTrigger()
        ...
        if (NpcListener.isAwaitingNpc()) {
            val awaitIndex = NpcListener.getAwaitingNodeIndex()
            val awaitNode = NpcListener.getAwaitingNode()
            ...
            if (awaitNode != null) {
                ...
                if (!intersectsNode(currentPos, nodeWorldPos, awaitNode.radius, awaitNode.height)) {
                    RouteUtils.debug("§c[NPC] Player left NPC await node #$awaitIndex - cancelling")
                    NpcListener.cancel()
                }
            }
        }
    ...
    private fun executeNode(node: WaypointNode, index: Int, room: Room?) {
        ...
        when {
            ...
            node.awaitNpc -> {
                RouteUtils.debug("§eNode #$index waiting for NPC interaction")
                NpcListener.startWaitingForNpc(node, index, room)
                RouteState.unlock()
            }
    ...
    private fun doNodeAction(node: WaypointNode, index: Int, room: Room?) {
        ...
        when (node.type) {
            ...
            WPType.ALIGN -> executeAlign(node, index, room)
        }
    }
    ...
    private fun executeAlign(node: WaypointNode, index: Int, room: Room?) {
        RouteUtils.debug("§b[Align] Starting alignment for node #$index")

        // AlignmentExecutor handles its own locking via RouteState.lock()
        // So we need to unlock first since executeNode already locked
        RouteState.unlock()

        AlignmentExecutor.startAlignment(node, index, room)
    }
