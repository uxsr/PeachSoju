package com.peachsoju.modules.impl.dungeon.autoroutes.data

import net.minecraft.core.BlockPos

enum class WPType {
    ETHER, AOTV, HYPE, SUPERBOOM, USEITEM, LOOK, NOP, WALK, STOP, ALIGN;

    companion object {
        fun fromString(str: String): WPType? = runCatching { valueOf(str.uppercase()) }.getOrNull()
    }
}
...
data class WaypointNode(
    ...
    fun copyWith(
        ...
        awaitType: String = this.awaitType, awaitDb: Boolean = this.awaitDb, awaitNpc: Boolean = this.awaitNpc,
        toBlock: BlockPos? = this.toBlock, targetBlock: BlockPos? = this.targetBlock, itemName: String? = this.itemName,
        mult: Int = this.mult, force: Boolean = this.force
    ): WaypointNode = WaypointNode(
        x, y, z, exact, type, yaw, pitch, chained, radius, height, start,
        delay, stop, center, awaitSecret, awaitBat, awaitType, awaitDb, awaitNpc, toBlock, targetBlock, itemName, mult, force
    )
}